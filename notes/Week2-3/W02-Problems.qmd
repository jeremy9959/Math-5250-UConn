...
2.  Let $G$ be a finite graph (i.e. a finite set of vertices $V$, and a finite set of edges $E\subset V\times V$ where we think of an edge $(v_1,v_2)$ as joining the vertices $v_1$ and $v_2$. For example:

![A graph with 5 vertices and 7 edges.](images/graph-5v7e.png){width=50%}

Now let $V^{0}$ be the space of real-valued functions $f:V\to \R$ and let $V^{1}$ be the space of real functions $f:E\to \R$.  
...
and all other $V^{j}$ are the zero vector space with zero maps.
